...
export class SysSettingsSecurityComponent implements OnInit {
  loading = false;
  i: SecuritySettingsDto;
  schema: SFSchema = {
    properties: {
      isEmailConfirmationRequiredForLogin: { type: 'boolean', title: '是否启用邮件激活' },

      userLockDivider: { type: 'string', title: '', ui: { widget: 'custom', grid: { spanLabelFixed: 0, span: 24 } } },
  ...
  save(value: any) {
    this.loading = true;
    let settings = SecuritySettingsDto.fromJS(value);
    this.configService.updateSecuritySettings(settings).subscribe(res => {
      this.loading = false;
